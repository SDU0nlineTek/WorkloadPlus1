...
def strEnc(data: str, *keys: str) -> str:
    ...
    return "".join(
        bt64ToHex(process_block(data[i : i + 4], keys)) for i in range(0, len(data), 4)
    )
...
def bt64ToHex(byteData: list) -> str:
    """64位二进制转16位十六进制"""
    return "".join(
        bt4ToHex("".join(map(str, byteData[i * 4 : i * 4 + 4]))) for i in range(16)
    )


def bt4ToHex(bt: str) -> str:
    """4位二进制转1位十六进制"""
    return f"{int(bt, 2):X}" if len(bt) == 4 and re.fullmatch("[01]{4}", bt) else ""
...
def getKeyBytes(key: str) -> list:
    """将字符串密钥转换为二进制块"""
    return [strToBt((key[i : i + 4] + "\0\0\0\0")[:4]) for i in range(0, len(key), 4)]
